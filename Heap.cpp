...
bool IsInTree(Node* node, int data, bool* check)
{
	if (node == NULL)
		return;
	...
	if(!*check)
		IsInTree(node->left, data, check);
	if (!*check)
		IsInTree(node->right, data, check);
}

void FindLeaf(Node* node, Node* target)
{
	if (node == NULL)
	{
		target = node;
	...
	if (node != NULL)
		FindLeaf(node->left, target);
	if (node != NULL)
		FindLeaf(node->right, target);
...
{
	bool check = false;
	if (IsInTree(tree->root, data, &check)) // 중복 허용하지 않음
	{
